...
# Custom guide-based split (arbitrary horizontal/vertical lines)
python -m image_splitter.cli test.png -p custom_splitter --set h_lines=[100,300,500] --set v_lines=[200,400]
...
# Custom splitter in chain (arbitrary guide positions)
...
The engine layer provides a Blender-inspired foundation with unified execution:
...
| `test_chain_as_graph_all.py` | ChainAsGraph vs CommandDispatcher pixel-identical for all 13 processors (22 tests) |
...
| `test_console_panel.py` | UI: interactive command console panel (customtkinter) |
| `test_custom_splitter.py` | Custom line splitter: simple, irregular, out-of-bounds, negative rejection |
| `test_data_blocks.py` | ImageDataBlock: name registry, version tracking, ref counting, clone, metadata (22 tests) |
...
| `test_execute_via_graph.py` | Unified execution path: _execute_via_graph() for single/multi-output and context |
| `test_gui_param_sync.py` | UI: GUI widget→state parameter synchronization data flow |
| `test_gui_smoke.py` | GUI initialization, file selection, processor switching, stop/cancel |
| `test_gui_state.py` | GuiState ViewModel: processor selection, progress tracking, param values |
...
| `test_legacy_adapter.py` | Legacy Adapter: ChainAsGraph vs CommandDispatcher pixel-identical output (15 tests) |
| `test_macro.py` | Macro recording: record, stop, generate script, playback, save |
| `test_new_processors.py` | New processor coverage: metadata P-mode, border styles, smart crop edge cases |
| `test_node_graph.py` | Node Graph + Evaluator: DAG topology, dirty propagation, cache, end-to-end (35 tests) |
| `test_operator_compliance.py` | Compliance audit + parameter contract for all processors |
| `test_param_widgets.py` | UI: shared parameter widget factory (customtkinter) |
| `test_pipeline_editor.py` | UI: visual pipeline chain editor (customtkinter) |
...
| `test_props.py` | Property System: coercion, validation, update callbacks, ui_metadata (30 tests) |
