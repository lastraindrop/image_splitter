...
def main():
    ...
    parser.add_argument("-j", "--jobs", type=int, default=os.cpu_count(), help="并行任务数 (默认: CPU核心数)")
    ...
    input_files = [f for f in unique_files if not str(f).startswith(str(output_dir))]
    ...
    with ProcessPoolExecutor(max_workers=args.jobs) as executor:
        ...
        futures = [
            executor.submit(
                split_image_core, 
                str(f), args.rows, args.cols, str(output_dir), args.template, offsets
