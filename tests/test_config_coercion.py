...
class TestConfigCoercion(unittest.TestCase):
    ...
    def test_unknown_type_raises_value_error(self):
        """P2-12 regression: unknown types must raise, not silently become str."""
        class _BadProcessor:
            def get_ui_metadata(self):
                return [{"name": "x", "label": "X", "type": "bogus_type", "default": 1}]
        processor = _BadProcessor()
        with self.assertRaises(ValueError) as ctx:
            coerce_processor_config(processor, {"x": "hello"})
        self.assertIn("Unknown parameter type", str(ctx.exception))

    def test_int_coercion_rejects_inf(self):
        """P2-13 regression: int coercion must reject 'inf' instead of
        raising an unhelpful OverflowError from int(float('inf'))."""
        processor = _DummyProcessor()
        with self.assertRaises(ValueError) as ctx:
            coerce_processor_config(processor, {"count": "inf"})
        self.assertIn("infinity", str(ctx.exception).lower())

    def test_int_coercion_rejects_nan(self):
        """P2-13 regression: int coercion must reject 'nan'."""
        processor = _DummyProcessor()
        with self.assertRaises(ValueError) as ctx:
            coerce_processor_config(processor, {"count": "nan"})
