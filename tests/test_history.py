...
class TestHistory(unittest.TestCase):
    ...
    def test_undo_returns_defensive_copy(self):
        """P2-16 regression: mutating the returned entry must NOT affect
        the redo stack.  undo/redo now return defensive copies so caller
        changes to config_snapshot or input_files are isolated."""
        self.manager.push(HistoryEntry(
            timestamp=1.0,
            operator_name="test",
            config_snapshot={"param": 1, "nested": [2, 3]},
            input_files=["a.png"],
            description="original",
        ))
        undone = self.manager.undo()
        self.assertIsNotNone(undone)
        # Mutate aggressively — the redo stack must stay pristine
        undone.config_snapshot["param"] = 999
        undone.config_snapshot["new_key"] = "injected"
        undone.input_files.append("malicious.txt")

        redone = self.manager.redo()
        self.assertIsNotNone(redone)
        self.assertEqual(redone.config_snapshot["param"], 1,
                         "caller mutation leaked into redo stack")
        self.assertNotIn("new_key", redone.config_snapshot,
                         "caller-injected key leaked into redo stack")
        self.assertEqual(redone.input_files, ["a.png"],
                         "caller mutation leaked into redo input_files")
