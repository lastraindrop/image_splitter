...
class TestProcessorsDeepDive(unittest.TestCase):
    ...
    def test_metadata_p_mode_colors_preserved(self):
        """P0-2 regression: P-mode image colors must be preserved after
        metadata strip.  The putpalette must happen BEFORE paste, otherwise
        PIL remaps source indices through the default (all-black) palette."""
        from image_splitter.processors.metadata import MetadataProcessor

        # Create a meaningful P-mode image with red pixels
        rgb_img = Image.new("RGB", (10, 10), color=(255, 0, 0))
        p_img = rgb_img.convert("P", palette=Image.Palette.ADAPTIVE)

        # Verify before: red pixel
        self.assertEqual(rgb_img.getpixel((5, 5)), (255, 0, 0))
        ...
        self.assertEqual(ctx["action"], "metadata_cleaned")

        # Convert back to RGB to verify color
        result_rgb = result_img.convert("RGB")
        r, g, b = result_rgb.getpixel((5, 5))
        self.assertEqual((r, g, b), (255, 0, 0),
                         f"P-mode colors corrupted: expected (255,0,0), "
                         f"got ({r},{g},{b})")
