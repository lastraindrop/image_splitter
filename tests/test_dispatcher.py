...
class TestDispatcherParsing(BaseTest):
    ...
    def test_parse_bare_identifier_as_string(self) -> None:
        """Ergonomic DSL: bare identifiers (enum/format/anchor values) are
        accepted as string values without requiring quotes.

        Previously `style=solid` failed because ast.literal_eval rejects
        bare Names.  This regressed the entire --chain feature for any
        processor taking string/enum parameters (border, format_converter,
        text_watermark anchor, geometry angle by name, ...).
        """
        ops = CommandDispatcher.parse_command(
            "border(width=8, color=red, style=solid)"
        )
        self.assertEqual(ops[0][1]["style"], "solid")
        self.assertEqual(ops[0][1]["color"], "red")
        self.assertEqual(ops[0][1]["width"], 8)

    def test_parse_bare_identifier_in_chain(self) -> None:
        """Bare identifiers work across chained operators."""
        ops = CommandDispatcher.parse_command(
            "geometry(rotate=90) | format_converter(format=WebP, quality=85)"
        )
        self.assertEqual(ops[0][1]["rotate"], 90)
        self.assertEqual(ops[1][1]["format"], "WebP")
        self.assertEqual(ops[1][1]["quality"], 85)

    def test_parse_bare_identifier_does_not_shadow_literals(self) -> None:
        """Numbers, lists, tuples, and quoted strings still parse as before."""
        ops = CommandDispatcher.parse_command(
            "custom_splitter(h_lines=[10, 20], v_lines=(30, 40), "
            "note='plain text') | resizer(width=0.5)"
        )
        self.assertEqual(ops[0][1]["h_lines"], [10, 20])
        self.assertEqual(ops[0][1]["v_lines"], (30, 40))
        self.assertEqual(ops[0][1]["note"], "plain text")
        self.assertEqual(ops[1][1]["width"], 0.5)


class TestDispatcherChain(BaseTest):
    """Chain execution with type coercion and defaults."""

    def test_execute_chain_delegates_to_chain_as_graph(self) -> None:
        img = Image.new("RGB", (10, 10))
        expected = [Image.new("RGB", (5, 5))]
        extra_config = {"height": 0.5}
        cmd = "resizer(width=0.5)"

        with patch(
            "image_splitter.engine.legacy_adapter.ChainAsGraph.execute_chain",
            return_value=expected,
        ) as execute_mock:
            result = CommandDispatcher.execute_chain(img, cmd, extra_config)

        self.assertIs(result, expected)
        execute_mock.assert_called_once_with(img, cmd, extra_config)
