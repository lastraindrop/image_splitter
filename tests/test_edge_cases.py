...
class TestProcessorEdgeCases(unittest.TestCase):
    ...
    def test_color_adjuster_p_mode_does_not_crash(self):
        """Regression: ImageEnhance cannot blend palette ('P') images and
        raised 'ValueError: image has wrong mode'.  The processor must
        convert P/1 modes to RGB before enhancement."""
        img_path = self.test_dir / "palette.png"
        Image.new("RGB", (40, 40), (255, 0, 0)).convert("P").save(img_path)
        success, msg = process_image(
            str(img_path), "color_adjuster",
            {"brightness": 1.5, "contrast": 1.2, "sharpness": 1.0,
             "color": 1.0, "output_dir": str(self.output_dir)},
        )
        self.assertTrue(success, msg)

    def test_color_adjuster_1bit_mode_does_not_crash(self):
        """Regression: same ImageEnhance failure for 1-bit ('1') images."""
        img_path = self.test_dir / "binary.png"
        Image.new("1", (40, 40)).save(img_path)
        success, msg = process_image(
            str(img_path), "color_adjuster",
            {"brightness": 1.3, "contrast": 1.0, "sharpness": 1.0,
             "color": 1.0, "output_dir": str(self.output_dir)},
        )
        self.assertTrue(success, msg)

    def test_filters_grayscale_preserves_rgba_alpha(self):
        """Regression: grayscale filter used convert('L').convert('RGB'),
        silently dropping the alpha channel and turning transparent PNGs
        opaque.  Alpha must now survive the grayscale conversion."""
        img_path = self.test_dir / "rgba.png"
        # Half-transparent red
        Image.new("RGBA", (30, 30), (255, 0, 0, 128)).save(img_path)
        success, msg = process_image(
            str(img_path), "filters",
            {"grayscale": True, "invert": False,
             "output_dir": str(self.output_dir)},
        )
        self.assertTrue(success, msg)
        # The single output file must still be RGBA (alpha preserved)
        outs = list(self.output_dir.glob("rgba*"))
        self.assertEqual(len(outs), 1, f"expected 1 output, got {outs}")
        with Image.open(outs[0]) as out:
            self.assertEqual(out.mode, "RGBA",
                             f"alpha lost: output mode is {out.mode}, expected RGBA")
