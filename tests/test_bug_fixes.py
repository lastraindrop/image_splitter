...
class TestBugFixes(unittest.TestCase):
    ...
    def test_bug03_geometry_rejects_invalid_angle(self):
        ...
        self.assertTrue(
            "Unsupported rotation angle" in msg or "not in options" in msg,
    ...
    def test_bug04_processors_init_exports_all(self):
        """All built-in processors must be importable from processors package."""
        from image_splitter.processors import (
            ...
            BorderProcessor, RoundedCornerProcessor, SmartCropProcessor,
        )
        classes = [
            GridSplitter, ImageResizer, CustomLineSplitter, CanvasAdjuster,
            ImageColorAdjuster, SimpleFilterProcessor, ImageFormatConverter,
        ...
        # Verify every imported class is a type with a valid name
        for cls in classes:
            self.assertTrue(issubclass(cls, object), f"{cls} is not a class")
            instance = cls()
            self.assertTrue(hasattr(instance, 'name'), f"{cls.__name__} has no name")
            self.assertTrue(hasattr(instance, 'display_name'), f"{cls.__name__} has no display_name")
        self.assertGreater(len(classes), 0, "No processor classes imported")
    ...
    def test_bug14_delete_keybind_does_not_override_keymap(self):
        ...
        try:
            ...
            self.assertLessEqual(
                len(root_delete_binds), 1,
                "root should have at most one <Delete> binding from keymap"
            )
        finally:
            root.destroy()
    ...
    def test_bug15_on_file_selected_opens_image_once(self):
        ...
        try:
            from image_splitter.gui import ImageSplitterApp
            app = ImageSplitterApp(root)
            app.state.current_files = [str(self.img_path)]
    ...
    def test_bug39_all_config_models_exported(self):
        ...
        for name in [
            ...
            "RoundedCornerConfig", "BorderConfig", "SmartCropConfig",
        ]:
            self.assertTrue(
    ...
    # ------------------------------------------------------------------
    # Regression tests for P0 bugs fixed during architecture audit
    # ------------------------------------------------------------------

    def test_reg_config_coercion_float_string_to_int(self):
        """BUG-48 regression: int("3.0") must not crash — float-string coercion."""
        from image_splitter.engine.config_coercion import _coerce_value
        meta = {"type": "int", "default": 1}
        result = _coerce_value(meta, "3.0")
        self.assertEqual(result, 3)
        self.assertIsInstance(result, int)

        # Also verify plain int strings still work
        result2 = _coerce_value(meta, "5")
        self.assertEqual(result2, 5)

        # And actual float values
        result3 = _coerce_value(meta, 2.7)
        self.assertEqual(result3, 2)

    def test_reg_config_coercion_none_falls_to_default(self):
        """BUG-48 regression: None value falls back to metadata default."""
        from image_splitter.engine.config_coercion import _coerce_value
        meta = {"type": "int", "default": 42}
        result = _coerce_value(meta, None)
        self.assertEqual(result, 42)

    def test_reg_config_coercion_none_no_default_returns_none(self):
        """BUG-48 regression: None with no default returns None (skip)."""
        from image_splitter.engine.config_coercion import _coerce_value
        meta = {"type": "int"}
        result = _coerce_value(meta, None)
        self.assertIsNone(result)

    def test_reg_data_block_duplicate_name_releases_old_image(self):
        """BUG-42 regression: creating a block with duplicate name releases old image."""
        from image_splitter.engine.data_blocks import ImageDataBlock
        ImageDataBlock.clear_all()
        b1 = ImageDataBlock(name="reg_dup", image=Image.new("RGB", (10, 10), "red"))
        self.assertTrue(b1.is_loaded)
        # Creating a new block with the same name should release b1's image
        b2 = ImageDataBlock(name="reg_dup", image=Image.new("RGB", (10, 10), "blue"))
        self.assertFalse(b1.is_loaded, "Old block's image should be released")
        self.assertTrue(b2.is_loaded, "New block should hold its image")
        self.assertIs(ImageDataBlock.get_by_name("reg_dup"), b2)
        ImageDataBlock.clear_all()

    def test_reg_core_template_error_no_cell_leak(self):
        """BUG-41 regression: template error should not leak already-opened cells."""
        # Use a template with an invalid placeholder to trigger the error path
        config = {
            "rows": 2,
            "cols": 2,
            "output_dir": str(self.output_dir),
            "template": "{filename}_{nonexistent_var}",
        }
    ...
    def test_reg_legacy_adapter_clear_all_selective(self):
        """BUG-53 regression: ChainAsGraph cleanup must not destroy user-created blocks."""
        from image_splitter.engine.data_blocks import ImageDataBlock
        from image_splitter.engine.legacy_adapter import ChainAsGraph
        ImageDataBlock.clear_all()

        # Create a user-owned block that must survive chain execution
        user_block = ImageDataBlock(
            name="user_owned_block",
            image=Image.new("RGB", (50, 50), "green"),
        )
        self.assertTrue(user_block.is_loaded)

        # Run a chain operation
        img = Image.new("RGB", (100, 100), "blue")
        results = ChainAsGraph.execute_chain(img, "resizer(width=0.5,height=0.5)")
        self.assertEqual(len(results), 1)

        # User block must still be alive after chain cleanup
        self.assertTrue(user_block.is_loaded,
                        "User-created block must survive chain cleanup")
        self.assertIsNotNone(ImageDataBlock.get_by_name("user_owned_block"))
        ImageDataBlock.clear_all()
        for r in results:
            r.close()

    def test_reg_registry_singleton_thread_safety(self):
        """BUG-45 regression: get_instance() must be thread-safe."""
        import threading
        from image_splitter.engine.registry import ProcessorRegistry
        ...
        def get_inst():
            try:
                inst = ProcessorRegistry.get_instance()
                instances.append(id(inst))
            except Exception as e:
                errors.append(e)

        threads = [threading.Thread(target=get_inst) for _ in range(50)]
        for t in threads:
            t.start()
        for t in threads:
            t.join()
    ...
    def test_reg_macro_sandbox_blocks_dangerous_builtins(self):
        """BUG-51/52 regression: macro sandbox must block __import__('os')."""
        from image_splitter.engine.macro import MacroPlayer
        # __import__ is overridden by _restricted_import which raises ImportError
        malicious_script = """
def run_macro(input_files, output_dir):
    os_module = __import__('os')
    return None
"""
        with self.assertRaises((ImportError, NameError)) as ctx:
            MacroPlayer.play_string(malicious_script, [], "./output")
        self.assertIn(
            "blocked",
            str(ctx.exception).lower(),
            f"Expected import blocking, got: {ctx.exception}"
        )

    def test_reg_macro_sandbox_blocks_open(self):
        """BUG-51/52 regression: macro sandbox must block open()."""
        from image_splitter.engine.macro import MacroPlayer
        # 'open' is not in _SAFE_BUILTINS; sandbox catches NameError
        # during run_macro() and returns ScriptResult(False, ...)
        malicious_script = """
def run_macro(input_files, output_dir):
    f = open('/etc/passwd')
    return None
"""
        result = MacroPlayer.play_string(malicious_script, [], "./output")
        self.assertFalse(result.success,
                         f"Expected sandbox to block open(), got: {result.message}")
        self.assertIn("open", result.message,
                      f"Expected 'open' in error message, got: {result.message}")

    def test_reg_macro_sandbox_allows_image_splitter_import(self):
        """BUG-51/52 regression: sandbox must allow image_splitter imports."""
        from image_splitter.engine.macro import MacroPlayer
        safe_script = """
from image_splitter.script_engine import ScriptEngine
...
def run_macro(input_files, output_dir):
    engine = ScriptEngine()
    return engine.process(input_files, 'grid_splitter',
                          {'rows': 1, 'cols': 1, 'output_dir': output_dir})
"""
        # The import succeeds; run_macro itself may raise for missing file,
        # but the import must NOT be blocked by the sandbox.
        try:
            result = MacroPlayer.play_string(safe_script, ["nonexistent.png"], "./output")
            # If it returned a result, the import was allowed (file error is expected)
            if not result.success:
                self.assertNotIn("blocked import", result.message.lower(),
                                 "image_splitter import should be allowed")
        except (FileNotFoundError, ValueError):
            pass  # Expected — file doesn't exist, but import was allowed
